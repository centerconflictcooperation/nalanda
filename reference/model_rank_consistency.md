...
themselves only produce continuous ratings, not ordinal ranks. Useful
for answering "Do models rank books the same way?"
...
- rank_within:
  ...
  Optional character vector of columns that define separate ranking
  contexts (e.g., `"group"`). Items are ranked independently within each
...
- concordance:
  ...
  `rank_within` combination (or one row total).

## Details

`unit_by` must identify exactly one row per model within each ranking
context. If the data still contain lower-level rows (for example,
