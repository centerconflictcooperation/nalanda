...
`run_structured_responses()` is the neutral one-model, one-prompt primitive;
`run_text_analysis()` remains its text-annotation-oriented wrapper.

For row-wise studies that cross several prompt variants with several model
routes, use `plan_prompt_grid()` to validate the configuration and preview
response counts, then `run_prompt_grid()` for provenance,
per-completion checkpoints, and resume support. Keep the raw results before
using `aggregate_model_forecasts()` to apply an explicit completion → prompt →
model → family weighting hierarchy. See the [multi-model structured forecasting
...
![](man/figures/README-function-choice.png)
