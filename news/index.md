...
- Fixed
  [`repair_run_ai_on_chapters()`](https://centerconflictcooperation.github.io/nalanda/reference/repair_run_ai_on_chapters.md)
  model selection so fully-qualified Portkey names match their
  normalized model labels in saved results.
- [`repair_run_ai_on_chapters()`](https://centerconflictcooperation.github.io/nalanda/reference/repair_run_ai_on_chapters.md)
  now retains Azure content-policy failures without retrying their
  unchanged prompts, while continuing to repair transient failures in
  the same result.
...
- Added fail-fast handling for unrecoverable model/integration route
  mismatches and unreachable gateway errors, so
  `run_ai_on_chapters(on_error = "skip")` no longer repeats the same
  backend or network failure across every simulation unit.
- Added
